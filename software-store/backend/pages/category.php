...
<div class="admin-layout">
    <aside class="sidebar">
        ...
        <ul class="sidebar-menu">
            ...
            <li class="menu-item active"><a href="category.php"><i class="fas fa-th-large"></i><span>分类管理</span></a></li>
    ...
    <div class="main-content">
        ...
        <div class="content-body fade-in">
            <div class="filter-bar">
                <input type="text" class="form-control search-input" id="searchKeyword" placeholder="搜索分类名称">
                <button class="btn btn-primary" id="btnSearch"><i class="fas fa-search me-1"></i>搜索</button>
                <button class="btn btn-success" id="btnAdd"><i class="fas fa-plus me-1"></i>添加分类</button>
            </div>
            ...
            <div class="modal-body">
                <form id="editForm">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" name="icon" id="editIcon" placeholder="如 fas fa-cog">
                        <small class="text-muted">参考：<a href="https://fontawesome.com/icons" target="_blank">Font Awesome 图标库</a></small>
                    </div>
                    <div class="mb-3">
...
$(function () {
    ...
    function loadList() {
        ...
        AdminApp.Api.get('category/list', params).then(function (res) {
    ...
    function renderList(list) {
        var $list = $('#dataList');
        if (list.length === 0) {
            $list.html('<tr><td colspan="8"><div class="empty-state"><i class="fas fa-th-large"></i><p>暂无分类数据</p></div></td></tr>');
            return;
        }
        var html = '';
        list.forEach(function (item) {
            ...
            html += '<tr>' +
                ...
                '<td><span class="badge bg-light text-dark">' + (item.software_count || 0) + '</span></td>' +
                '<td>' + (item.sort || 0) + '</td>' +
                ...
                '<button class="btn-action delete btn-delete" data-id="' + item.id + '" data-name="' + AdminApp.UI.escape(item.name) + '" data-action="category/delete" title="删除"><i class="fas fa-trash"></i></button></td>' +
                '</tr>';
        });
        $list.html(html);
    }
    ...
    $(document).on('click', '.btn-action.edit', function () {
        var id = $(this).data('id');
        // 从当前列表中取数据
        AdminApp.Api.get('category/list').then(function (res) {
            if (res.code === 200) {
                var item = (res.data || []).find(function (c) { return parseInt(c.id, 10) === parseInt(id, 10); });
                if (item) {
    ...
    $('#btnSave').on('click', function () {
        ...
        if (!data.name) { AdminApp.UI.toast('请输入分类名称', 'warning'); return; }
        var action = id ? 'category/update' : 'category/create';
        if (id) data.id = id;
    ...
    $('#searchKeyword').on('keypress', function (e) { if (e.which === 13) loadList(); });
    AdminApp.bindDelete('category/delete', loadList);
    loadList();
});
</script>
</body>
